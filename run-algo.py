...
def main(args):
    ...
    for file in path.glob("test-*.parquet"):
        ...
        try:
            ...
            recs.to_parquet(dest / f'recs-{suffix}', index=False, compression='brotli')
            ...
            if isinstance(algo, Predictor) and not args['--no-predict']:
                ...
                preds.to_parquet(dest / f'pred-{suffix}', index=False, compression='brotli')
        finally:
            model.close()
